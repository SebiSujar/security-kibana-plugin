...
module.exports = function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT) {
    ...
    server.route({
        method: 'GET',
        path: `${APP_ROOT}${routesPath}capture-url-hash`,
        options: {
            auth: false
        },
          ...
          return h.response(
              ...
              <script>
                  window.location.replace(
                      '${basePath}${routesPath}login?nextUrl=' + encodeURIComponent('${nextUrl}' + window.location.hash)
                  );
              </script>
            `
          ).type('text/html')
    ...
    server.route({
        ...
        async handler(request, h) {
            console.log("SEBI - entre al login");
            if (request.auth.isAuthenticated) {
                console.log("SEBI - ya estaba autenticado :P");
            ...
            console.log("SEBI - Mi nextUrl es: " + nextUrl);
        ...
        handler: async (request, h) => {
            ...
            try {
                ...
                if (nextUrl) {
                    ...
                    console.log("SEBI - redirijo a:" + nextUrl);
                    return h.redirect(nextUrl);
                }

                console.log("SEBI - redirijo a kibana :(");
                return h.redirect(basePath + '/app/kibana');
