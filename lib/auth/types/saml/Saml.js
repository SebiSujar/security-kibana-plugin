...
export default class Saml extends AuthType {
    ...
    async authenticate(credentials, options = {}, additionalAuthHeaders = {})  {
        ...
        try {
            ...
            let user = await this.server.plugins.opendistro_security.getSecurityBackend().authenticateWithHeader(this.authHeaderName, credentials.authHeaderValue, additionalAuthHeaders);
    ...
    onUnAuthenticated(request, h, error) {
        ...
        console.log("SEBI - hago redirect con nextUrl:" + nextUrl)
        return h.redirect(`${this.basePath}/auth/saml/capture-url-hash?nextUrl=${nextUrl}`);
